...
async def run_website_scanner(
    ...
    async with AsyncClient() as client:
        try:
            ...
            return await run_website_scanner(session, website, recipient_email, max_pages, delay, concurrent)

        except WebConnectionError as e:
            logger.error(f"Lost connection, waiting and trying again. {e}")
            await asyncio.sleep(2 * 60)  # TODO  maybe test a fetch and wait till open
            return await run_website_scanner(session, website, recipient_email, max_pages, delay, concurrent)
